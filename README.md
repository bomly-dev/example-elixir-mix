...
> Find vulnerabilities, trace reachability, and audit open-source risk in your **Elixir** project.

## About

This repository is a [Bomly](https://bomly.dev) example project for **Elixir** with **Mix**.
...
```bash
bomly scan --url https://github.com/bomly-dev/example-elixir-mix --ecosystems elixir
```
...
```bash
bomly scan --url https://github.com/bomly-dev/example-elixir-mix --ecosystems elixir --enrich --audit
```

### 3. Trace why a dependency exists

Understand every path that pulls `plug` into your graph:

```bash
bomly explain plug --url https://github.com/bomly-dev/example-elixir-mix
```
